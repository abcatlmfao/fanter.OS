...
var ratingsLoaded = false, gamesLoaded = false;
...
async function syncGamesToBin(data) {
  ...
  try {
    var binData = { games: data.map(g => ({ name: g.name, desc: g.desc || getDefaultDescription(g.category), category: g.category, url: g.url })), lastUpdated: new Date().toISOString() };
...
// ===== REVIEWS =====
async function loadGlobalReviews() { try { var r = await fetch(`https://api.jsonbin.io/v3/b/${REVIEWS_BIN_ID}/latest`, { headers: { 'X-Master-Key': BIN_API_KEY } }), d = await r.json(); if (d.record?.reviews) globalReviews = d.record.reviews; console.log('✅ Reviews loaded'); } catch (e) {} }
...
async function loadGlobalRatings() {
  try {
    var r = await fetch(`https://api.jsonbin.io/v3/b/${RATINGS_BIN_ID}/latest`, { headers: { 'X-Master-Key': BIN_API_KEY } }), d = await r.json();
    if (d.record?.ratings) globalRatings = d.record.ratings;
    console.log('✅ Ratings loaded:', Object.keys(globalRatings).length, 'games');
    ratingsLoaded = true;
    if (gamesLoaded) tryDisplayGames();
    if (window.updateStarDisplays) window.updateStarDisplays();
  } catch (e) { console.error('Ratings failed:', e); ratingsLoaded = true; if (gamesLoaded) tryDisplayGames(); }
}
...
// ===== TRY DISPLAY GAMES (WAITS FOR BOTH) =====
function tryDisplayGames() {
  if (window.gamesData?.length) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.displayFilteredGames = function(games) {
    ...
    games.forEach(g => {
      ...
      var avg = '0.0';
      if (globalRatings[g.name] && globalRatings[g.name].average != null && typeof globalRatings[g.name].average === 'number') avg = globalRatings[g.name].average.toFixed(1);
      var img = g.image?.startsWith('http') ? g.image : 'https://via.placeholder.com/200x113?text=No+Image';
  ...
  function updateStarDisplays() {
    document.querySelectorAll('.game').forEach(c => {
      ...
      if (t && globalRatings[n] && globalRatings[n].average != null) t.textContent = globalRatings[n].average.toFixed(1);
    });
  }
  ...
  function attachGameCardEvents() {
    ...
    document.querySelectorAll('.game-star').forEach(s => s.onclick = e => { e.stopPropagation(); var c = s.closest('.game'), n = c.dataset.gameName, v = parseInt(s.dataset.value); if (n) { submitRating(n, v); c.querySelectorAll('.game-star').forEach((st, i) => st.classList.toggle('active', i < v)); var t = c.querySelector('.game-rating-text'); if (t && globalRatings[n] && globalRatings[n].average != null) t.innerHTML = globalRatings[n].average.toFixed(1); } });
  ...
  fetch("./config/games.json").then(r => r.json()).then(d => {
    window.gamesData = d; gamesLoaded = true;
...
function showGameDetailsModal(n, u, img, desc, cat, load, dev, rel) {
  ...
  var plays = window.gamePlayCounts[n] || 0, earned = window.gameEarnings[n] || 0, favs = JSON.parse(localStorage.getItem("favourites")||"[]"), isFav = favs.includes(n);
  var ur = (typeof userVotes !== 'undefined' && userVotes[n]) || 0, avg = '0.0', cnt = 0;
  if (globalRatings[n] && globalRatings[n].average != null) { avg = globalRatings[n].average.toFixed(1); cnt = globalRatings[n].count; }
  ...
  var sel = 0;
  modal.querySelectorAll('.review-star').forEach(s => { s.onmouseenter = () => { var v = parseInt(s.dataset.value); modal.querySelectorAll('.review-star').forEach((x, i) => x.style.color = i < v ? '#ffcc00' : 'rgba(255,255,255,0.2)'); }; s.onmouseleave = () => modal.querySelectorAll('.review-star').forEach((x, i) => x.style.color = i < sel ? '#ffcc00' : 'rgba(255,255,255,0.2)'); s.onclick = () => { sel = parseInt(s.dataset.value); modal.querySelectorAll('.review-star').forEach((x, i) => x.style.color = i < sel ? '#ffcc00' : 'rgba(255,255,255,0.2)'); }; });
...
setTimeout(() => tryDisplayGames(), 100);
setTimeout(() => tryDisplayGames(), 500);
setTimeout(() => tryDisplayGames(), 1000);
